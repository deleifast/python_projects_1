...
def pdv():
		...
		if os.path.isfile(PATHPDV):
			...
			dest_dir = "C:\\PDV"
			for file in glob.glob(r'X:/loja36_pdv.rar'):
				print(file)
			shutil.copy(file, dest_dir)
			logging.info("Copiado arquivo: " + file)

		PATHPDV='C://PDV/loja36_pdv.rar'
		...
		PATHPLUGIN='X://loja36_plugin.rar'
		...
		if os.path.isfile(PATHPLUGIN):
			...
			for file in glob.glob(r'X:/loja36_plugin.rar'):
				print(file)
			shutil.copy(file, dest_dir)
			logging.info("Copiado arquivo: " + file)
		...
		PATHPLUGIN='C://PDV/PLUGIN/loja36_plugin.rar'
		...
		if os.path.isfile(PATHPLUGIN):
			...
			path = 'c://pdv/plugin/loja36_plugin.rar'
		...
		try:
			...
			msg.pack_forget()				
			msg = Label(gui.root, text='Sem arquivo socket_cliente36.exe', fg = "red", font= 'Arial 18')
			msg.pack()
			logging.info('Sem arquivo socket_cliente36.exe')
			time.sleep(5)
...
gui = GUI_Core()
gui.root.title("REMARCA/NAGUMO - lj36 - 4.6")
gui.root.mainloop()
