#encoding: iso-8859-1
import os, shutil, subprocess, time, glob, copy, os.path, sys, socket, threading, datetime, logging, configparser, platform
...
class GUI_Core(object):
    ...
    def __init__(self):
        ...
        self.msg = Label(self.root, text='REMARCA/NAGUMO - Versão 4.6S - Lj08', fg="red", font='Arial 8')
...
def pdv():
		...
		time.sleep(0.1)
		msg = Label(gui.root, text='Iniciando, aguarde....', fg = "green", font= 'Arial 50')
		...
		PATHPDV='X://loja08s_pdv.rar'

		if os.path.isfile(PATHPDV):

			time.sleep(5)
			msg.pack_forget()
			msg = Label(gui.root, text='Copiando arquivos para pasta PDV...', fg="blue", font= 'Arial 45')
			msg.pack(fill=BOTH, anchor=N, expand=True)	
			logging.info('Copiando arquivos para pasta PDV...')
			
			dest_dir = "C:\\PDV"
			for file in glob.glob(r'X:/loja08s_pdv.rar'):
				print(file)
			shutil.copy(file, dest_dir)
			logging.info("Copiado arquivo: " + file)

		PATHPDV='C://PDV/loja08s_pdv.rar'
		...
		if os.path.isfile(PATHPDV):
			...
			time.sleep(5)
			msg.pack_forget()
			msg = Label(gui.root, text="Extraindo todos os arquivos para pasta pdv agora...", fg="blue", font= 'Arial 40')
			msg.pack(fill=BOTH, anchor=N, expand=True)	
			...
			for raiz, diretorios, arquivos in os.walk('C:/PDV'):
				for arquivo in arquivos:
					if arquivo.endswith('loja08s_pdv.rar'):
			...
			time.sleep(5)
			msg.pack_forget()
			msg = Label(gui.root, text="Sem atualização na pasta PDV.", fg="blue", font= 'Arial 50')
			msg.pack(fill=BOTH, anchor=N, expand=True)	
			logging.info("Sem arquivos para a pasta PDV.")
      
		PATHPLUGIN='X://loja08s_plugin.rar'

		if os.path.isfile(PATHPLUGIN):

			time.sleep(5)
			msg.pack_forget()
			msg = Label(gui.root, text='Copiando arquivos para pasta \PDV\PLUGIN, aguarde....', fg="blue", font= 'Arial 40')
			msg.pack(fill=BOTH, anchor=N, expand=True)	
			logging.info('Copiando arquivos para pasta PDV\PLUGIN, aguarde....')
			
			dest_dir = "C:\\PDV\PLUGIN"
			for file in glob.glob(r'X:/loja08s_plugin.rar'):
				print(file)
			shutil.copy(file, dest_dir)
			logging.info("Copiado arquivo: " + file)
		...
		PATHPLUGIN='C://PDV/PLUGIN/loja08s_plugin.rar'
		...
		if os.path.isfile(PATHPLUGIN):
			...
			time.sleep(5)
			msg.pack_forget()
			msg = Label(gui.root, text="Sem atualização na pasta \PDV\PLUGIN...", fg="blue", font= 'Arial 50')
			msg.pack(fill=BOTH, anchor=N, expand=True)	
		...
		if os.path.exists("work"):
			...
			time.sleep(5)
			msg.pack_forget()
			msg = Label(gui.root, text="Não encontrada pasta WORK", fg="red", font= 'Arial 50')
			msg.pack(fill=BOTH, anchor=N, expand=True)	
		...
		msg.pack_forget()				
		msg = Label(gui.root, text='Aguarde....', fg = "blue", font= 'Arial 50')
		msg.pack(fill=BOTH, anchor=N, expand=True)
		logging.info('Aguarde....')
		time.sleep(5)
		...
		if os.path.exists("dllsat32bits.dll"):
			msg.pack_forget()
			msg = Label(gui.root, text='SAT Dimep, aguarde....', fg="blue", font= 'Arial 50')
			msg.pack(fill=BOTH, anchor=N, expand=True)
			...
			msg.pack_forget()
			msg = Label(gui.root, text='SAT Sweda, aguarde....', fg="blue", font= 'Arial 50')
			msg.pack(fill=BOTH, anchor=N, expand=True)
			...
			msg.pack_forget()
			msg = Label(gui.root, text='SAT Jetway, aguarde....', fg="blue", font= 'Arial 50')
			msg.pack(fill=BOTH, anchor=N, expand=True)
			...
			msg.pack_forget()
			msg = Label(gui.root, text='SAT Crt_ID, aguarde....', fg="blue", font= 'Arial 50')
			msg.pack(fill=BOTH, anchor=N, expand=True)
		...
		msg.pack_forget()
		msg = Label(gui.root, text='Processos inicializados com sucesso!', fg="blue", font= 'Arial 45')
		msg.pack(fill=BOTH, anchor=N, expand=True)	
...
#gui.root.title("REMARCA/NAGUMO - VersÃ£o 2.2S - Lj11")
