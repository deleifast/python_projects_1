...
def pdv():
		...
		if os.path.isfile(PATHPDV):
			...
			dest_dir = "C:\\PDV"
			for file in glob.glob(r'X:/loja53_pdv.rar'):
				print(file)
			shutil.copy(file, dest_dir)
			logging.info("Copiado arquivo: " + file)

		PATHPDV='C://PDV/loja53_pdv.rar'
		...
		if os.path.isfile(PATHPDV):
			...
			for raiz, diretorios, arquivos in os.walk('C:/PDV'):
				for arquivo in arquivos:
					if arquivo.endswith('loja53_pdv.rar'):
		...
		PATHPLUGIN='X://loja53_plugin.rar'
		...
		if os.path.isfile(PATHPLUGIN):
			...
			for file in glob.glob(r'X:/loja53_plugin.rar'):
				print(file)
			shutil.copy(file, dest_dir)
			logging.info("Copiado arquivo: " + file)
		...
		PATHPLUGIN='C://PDV/PLUGIN/loja53_plugin.rar'
		...
		try:
			os.startfile(r"socket_cliente53.exe")
			logging.info("Chamado socket_cliente53.exe")
		except FileNotFoundError:
			msg.pack_forget()				
			msg = Label(gui.root, text='Sem arquivo socket_cliente53.exe', fg = "red", font= 'Arial 18')
			msg.pack()
			logging.info('Sem arquivo socket_cliente53.exe')
			time.sleep(5)
...
gui = GUI_Core()
gui.root.title("REMARCA/NAGUMO - lj53 - 4.6")
gui.root.mainloop()
