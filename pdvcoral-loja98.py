...
def pdv():
		...
		PATHPDV='X://loja98_pdv.rar'
		PATHPLUGIN='X://loja98_plugin.rar'

		if os.path.isfile(PATHPDV):
			
			dest_dir = "C:\\PDV"
			for file in glob.glob(r'X:/loja98_pdv.rar'):
				print(file)
		...
		if os.path.isfile(PATHPLUGIN):
			...
			for file in glob.glob(r'X:/loja98_plugin.rar'):
				print(file)
			shutil.copy(file, dest_dir)
			logging.info("Copiado arquivo: " + file)
		...
		PATHPDV='C://PDV/loja98_pdv.rar'
		PATHPLUGIN='C://PDV/PLUGIN/loja98_plugin.rar'
		...
		if os.path.isfile(PATHPLUGIN):
			...
			path = 'c://pdv/plugin/loja98_plugin.rar'
...
gui = GUI_Core()
gui.root.title("REMARCA/NAGUMO - lj98 - 4.5")
gui.root.mainloop()
