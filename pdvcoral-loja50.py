...
def pdv():
        ...
        if os.path.isfile(PATHPLUGIN):
            ...
            path = 'c://pdv/plugin/loja50_plugin.rar'
        ...
        try:
            subprocess.run("C:\\pdv\\ventana.bat")
            msg.pack_forget()
            msg = Label(gui.root, text="Abrindo programa VentanaRF_Central.", fg="blue", font='Arial 18')
            msg.pack()
            logging.info("Chamado VentanaRF")
            time.sleep(3)            
        except:
            msg.pack_forget()
            msg = Label(gui.root, text="Sem arquivo ventana.bat, favor verificar!!", fg="red", font='Arial 18')
            msg.pack()
            logging.info("Não encontrado arquivo ventana.bat na pasta PDV")
            time.sleep(5)
        ...
        if os.path.exists("socket_cliente.exe"):
            os.startfile(r"socket_cliente.exe")
            logging.info("Chamado socket_cliente.exe")
        else:
            pass
            logging.info('Sem arquivo socket_cliente.')
...
gui = GUI_Core()
gui.root.title("REMARCA/NAGUMO - lj50 - 4.6")
gui.root.mainloop()
