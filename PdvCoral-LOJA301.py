...
def pdv():
		...
		PATHPDV='X://loja301_pdv.rar'
		PATHPLUGIN='X://loja301_plugin.rar'

		if os.path.isfile(PATHPDV):
			
			dest_dir = "C:\\PDV"
			for file in glob.glob(r'X:/loja301_pdv.rar'):
				print(file)
		...
		if os.path.isfile(PATHPLUGIN):
			...
			for file in glob.glob(r'X:/loja301_plugin.rar'):
				print(file)
			shutil.copy(file, dest_dir)
		...
		PATHPDV='C://PDV/loja301_pdv.rar'
		PATHPLUGIN='C://PDV/PLUGIN/loja301_plugin.rar'
...
#			destination = 'C://pdv/versões_anteriores_'+(t)
			...
			file_pdv = "C:\\pdv\loja301_pdv.rar"
			def unrar(file):
			...
			for raiz, diretorios, arquivos in os.walk('C:/PDV'):
				for arquivo in arquivos:
					if arquivo.endswith('loja301_pdv.rar'):
		...
		if os.path.isfile(PATHPLUGIN):
			...
			file_plugin = "C:\\pdv\plugin\loja301_plugin.rar"
			def unrar(file):
				rf=rarfile.RarFile(file)
			...
			for raiz, diretorios, arquivos in os.walk('C:/PDV'):
				for arquivo in arquivos:
					if arquivo.endswith('loja301_plugin.rar'):
					   os.remove(os.path.join(raiz, arquivo))
		...
		if os.path.exists('key.bat'):
			call('key.bat')
		else:
			with open("key.txt", "w") as stream:
				print('KEY.BAT NÃO EXISTE, VERIFICAR!', file=stream)
...
gui.root.title("REMARCA/NAGUMO - lj301 - 4.0")
gui.root.mainloop()
